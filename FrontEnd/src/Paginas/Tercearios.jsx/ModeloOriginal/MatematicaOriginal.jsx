// src/Paginas/Tercearios.jsx/ModeloOriginal/MatematicaOriginal.jsx
...
const questoesExemplo = [
  {
    id: 1,
    titulo: "Problema de Álgebra",
    descricao: "Resolva a equação quadrática: 2x² - 5x - 3 = 0. Mostre todos os passos da solução.",
    dificuldade: "Médio",
    pontos: 15
  },
  {
    ...
    descricao: "Calcule a área de um triângulo equilátero com lado medindo 8cm. Apresente a fórmula utilizada.",
    dificuldade: "Fácil",
    pontos: 10
  },
  {
    ...
    descricao: "Encontre a derivada da função f(x) = 3x⁴ - 2x³ + 5x - 7. Explique cada passo da derivação.",
    dificuldade: "Difícil",
    pontos: 20
  }
];
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      <div className="bg-green-600 text-white shadow-md">
        <div className="flex items-center justify-between p-4">
          <button 
            onClick={() => navigate("/")} 
            className="flex items-center gap-1 border border-white px-2 py-1 text-[10px] sm:px-3 sm:py-1.5 sm:text-xs md:px-4 md:py-2 md:text-sm rounded hover:bg-white hover:text-green-600 transition"
          >
          ...
          <div className="flex items-center gap-2">
            <div className="bg-white text-green-600 font-bold px-2 py-1 text-[10px] sm:px-3 sm:py-1.5 sm:text-xs md:px-4 md:py-2 md:text-sm rounded-full flex items-center gap-1 shadow-md">
        ...
        <div className="w-full h-3 bg-white/30">
          <div 
            className="h-3 transition-all duration-1000 bg-gradient-to-r from-green-400 to-emerald-500"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden lg:block w-80 bg-black text-white shadow-lg p-3 overflow-y-auto" translate="no">
          <h2 className="text-xl font-bold mb-4 text-center border-b border-gray-700 pb-1">Ranking de Matemática</h2>
              ...
              {ranking.length > 0 ? (
                ranking.map((participanteRank) => (
                  <tr key={participanteRank.id} className="border-b border-gray-700 hover:bg-gray-900">
                    ...
                    <td className="px-2 py-2">
                      <div className="flex items-center gap-2">
                        {participanteRank.usuario?.imagem ? (
                          ...
                          <div className="w-7 h-6 rounded-full bg-green-500 flex items-center justify-center text-white text-xs">
                    ...
                    <td className="px-2 py-2 text-green-400 font-semibold">{participanteRank.pontuacao || 0}</td>
                  </tr>
                ))
              ) : (
                <tr>
        ...
        <div className="flex-1 flex flex-col items-center p-4 overflow-auto space-y-4" translate="no">
          ...
          {/* ENUNCIADO */}
          <div className="w-full max-w-4xl bg-gradient-to-r from-green-50 to-emerald-100 border-l-4 border-green-600 rounded-xl shadow p-4">
            <h3 className="text-lg font-bold text-green-700 mb-2">
              {questoes[questaoIndex]?.titulo || `Problema ${questaoIndex + 1}`}
            </h3>
            ...
            <div className="mt-2 text-xs text-gray-500">
              Dificuldade: <span className="font-semibold">
                {questoes[questaoIndex]?.dificuldade || 'Média'}
              </span>
          ...
          <div className="w-full max-w-4xl bg-white rounded-xl shadow-md p-4 space-y-3">
            ...
            <div className="flex gap-1 md:gap-2 flex-wrap justify-center border-b pb-3 overflow-x-auto">
              {["+", "-", "×", "÷", "√", "(", ")", "²", "³", "=", "π", "°", "≤", "≥", "≠", "≈", "∫", "∑", "∞"].map((op) => (
                <button 
                  ...
                  className="px-2 py-1 text-xs md:text-sm bg-gray-100 hover:bg-green-100 text-gray-800 rounded-md transition"
                >
                  {op}
                </button>
              ))}
            ...
            <textarea 
              ...
              className="w-full h-80 resize-none p-3 text-sm md:text-base bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
          ...
          <div className="w-full max-w-4xl bg-white rounded-xl shadow-md p-4">
            ...
            <div className="w-full h-3 rounded-full bg-gray-200 overflow-hidden">
              <div 
                className={`h-full transition-all duration-300 ${questaoTime < 10 ? "bg-red-500" : "bg-green-600"}`}
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-all duration-300 w-full sm:w-auto sm:min-w-[160px] bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-700 hover:to-emerald-800 text-white shadow-md hover:shadow-lg active:scale-95"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span className="text-sm md:text-base">Executar Resolução</span>
          </button>

          {/* RESULTADO / AVALIAÇÃO */}
          {resultado && (
            <div className="w-full max-w-4xl bg-white rounded-xl shadow-md p-4 border-l-4 border-green-600">
          ...
          <div className="flex flex-col sm:flex-row w-full max-w-5xl justify-between gap-3 mt-4 lg:hidden">
            ...
            <button 
              ...
              className="flex-1 border-2 border-green-600 text-green-600 hover:bg-green-600 hover:text-white px-4 py-3 rounded-lg transition-colors text-sm flex items-center justify-center gap-2"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="hidden lg:flex w-64 bg-white text-gray-800 shadow-lg p-4 overflow-y-auto flex-col items-center space-y-3">
          ...
          <div className="flex flex-col items-center mb-3">
            {participante?.usuario?.imagem ? (
              <img 
                ...
                className="w-20 h-20 rounded-full object-cover border-2 border-green-400 mb-2"
              />
            ) : (
              <div className="w-20 h-20 rounded-full mb-2 border-2 border-green-400 bg-gradient-to-r from-green-500 to-emerald-700 flex items-center justify-center text-white text-2xl font-bold">
          ...
          {participante ? (
            <div className="w-full flex flex-col gap-4 items-center">
              {[
                { valor: participante.pontuacao || 0, label: "Pontuação", cor: "#10b981", max: 10000 },
                { valor: participante.posicao || 0, label: "Posição", cor: "#10b981", max: 100 },
                { valor: participante.casos_resolvidos || 0, label: "Casos Resolvidos", cor: "#10b981", max: 100 }
              ].map((item, idx) => (
                <div key={idx} className="bg-white rounded-3xl border border-green-200 p-2 flex flex-col items-center gap-1 w-40">
